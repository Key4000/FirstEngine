...
namespace FirstEngine{
    ...
    //количество компонентов по типу 
    ...
    //кол-во байт которое занимает шейдерный тип 
    ...
    //конвертируем внутренний тип в OpenGL тип
    ...
    constexpr GLenum usage_to_GLenum(
    ...
    //конструктор элемента по внутреннему типу
    ...
    {
        glGenBuffers(1, &m_id);                                                 //(колво буферов, ссылка) - генератор буфера
        glBindBuffer(GL_ARRAY_BUFFER, m_id);                                    //выбрать текущий буфер
        glBufferData(GL_ARRAY_BUFFER, size, data, usage_to_GLenum(usage));      //передаем данные из ОП в видеопамять
    }
    //деструктор 
    ...
    //оперататор присваивания с перемещением не выбрасывает исключений 
    VertexBuffer& VertexBuffer::operator=(VertexBuffer&& vertex_buffer) noexcept
    {
        //присваиваем id буффера 
