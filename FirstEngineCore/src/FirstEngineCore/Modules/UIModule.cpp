...
namespace FirstEngine {
	...
	void UIModule::on_window_create(GLFWwindow* pWindow)
	{
		...
		io.ConfigFlags |= ImGuiConfigFlags_::ImGuiConfigFlags_DockingEnable;  //устанавливаем флаги для докинга 
		io.ConfigFlags |= ImGuiConfigFlags_::ImGuiConfigFlags_ViewportsEnable;  //флаг для viewport
		ImGui_ImplOpenGL3_Init();   //opengl для imgui
		ImGui_ImplGlfw_InitForOpenGL(pWindow, true); //инициализируем бэкенд imgui для связки с glfw для openGl(можно и vulkan)
	}
